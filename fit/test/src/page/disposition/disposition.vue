  ...
  <div class="disposition">
    ...
    <comfire-dialog v-model="comDialog" :sure="commit">
      ...
      <p class="comText">共<span>{{commitList.length}}</span>只老虎参加PK需支付<span>{{commitList.length*(chooseType?$store.state.pkShenPrice:$store.state.pkEntryPrice)}}</span>FMVP报名费？</p>
		...
		<animate-info :Ani="ani" :bothInfo="bothInfo" @closeAni="closeAni" ></animate-info>
...
<script>
  ...
  import RedTable from "../../components/red-table";
	import AnimateInfo from './recordAnimate.vue';
  export default{
    components: {
      RedTable,
      TigerImg,
    ...
    methods:{
      ...
      getTigerList:function (type) {
        ...
        this.$axios.post('/api/battle/character/horselist/',data).then(
          res=>{
            let result = res.data;
            if(result.IsSuccess){
      ...
      getTime:function () {
        this.$axios.get('/api/battle/characterlastsettlingtime/').then(
          res=>{
            let result = res.data;
            if(result.IsSuccess){
      ...
      getDetail:function (TeamKey) {
        ...
        this.$axios.post('/api/battle/character/teamdetail/',data).then(
          res=>{
            let result = res.data;
            if(result.IsSuccess){
							if(result.Result.opponentNick!=null){
								this.bothInfo=result.Result;
							}
      ...
      commit:function () {
        ...
        this.$axios.post('/api/horse/battle/character/',data).then(
          res=>{
            let result = res.data;
            if(result.IsSuccess){
                if(this.timeType)
