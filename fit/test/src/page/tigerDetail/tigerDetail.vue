      ...
      <div   :style="setScrollMinHeight()">
        ...
        <div class="Info" >
          <div  style="height: 100%" flex="dir:top main:center">
            <p flex="cross:center"><span class="ssr">{{tigerDetail.RareLevel|rare}}</span><span>(等级:{{lv(tigerDetail.GradeLevel)}})</span></p>
            <p style="font-size: 0.42677rem;line-height: 0.7rem">{{getType(tigerDetail)}}#{{tigerDetail.HorseNo}}</p>
						<div>
						<div class="exText">经验:</div>
						<div class="exContext">
							<div class="exTextPosition">{{ex(tigerDetail)}}</div>
							<div class="exEffective" :style="{width:exLength}">
							</div>
						</div>
						</div>
            <p flex="main:justify"><!--  class="num-abc-warp"  -->
							<span>战斗力:{{tigerDetail.CombatPower}}</span>
							<span>耐力:{{tigerDetail.Endurance}}</span>
							<span>敏捷:{{tigerDetail.Agile}}</span>
							<span>生命力:{{tigerDetail.life}}</span>
						</p>
          </div>
  ...
  export default{
    ...
    methods:{
      ...
      getSynthesisList:function (type) {
        ...
        this.$axios.post('/api/activity/gettransferhorses/', data).then(
          res => {
            let result = res.data;
            if (result.IsSuccess) {
      ...
      Synthesis:function (steedKey) {
        ...
        this.$axios.post('/api/activity/transferfamily/',data).then(
          res=>{
            let result= res.data;
            if(result.IsSuccess)
            {
      ...
      getDetailInfo: function (key,cancle) {
        let data = {key: key};
        this.$axios.get('/api/horse/get/?Key=' + key).then(
          res => {
            let result = res.data;
            if (result.IsSuccess) {
...
//          this.openSellType();
      ...
      SellOut: function () {
        ...
        this.$axios.post('/api/horse/trade/cancel/', data).then(
          res => {
            let result = res.data;
            if (result.IsSuccess) {
              this.$tips("下架成功");
      ...
      sureToAuction: function () {
        ...
        this.$axios.post('/api/horse/trade/userauctionsale/', data).then(
          res => {
            let result = res.data;
            if (result.IsSuccess) {
              this.$tips("拍卖上架成功");
...
//              this.tigerDetail.TradeStatus = false;
      ...
      sureToSell: function () {
        ...
        this.$axios.post('/api/horse/trade/sale/', data).then(
          res => {
            let result = res.data;
            if (result.IsSuccess) {
              this.$tips("上架成功");
      ...
      sureToBorn: function () {
        ...
        let data = {horseId: this.tigerDetail.HorseKey};
        this.$axios.post('/api/horse/breed/', data).then(
          res => {
            let result = res.data;
            if (result.IsSuccess) {
              this.getDetailInfo(this.tigerDetail.HorseKey,'cancle');
      ...
      sureToFeed:function () {
        ...
        let data = {horseId: this.tigerDetail.HorseKey};
        this.$axios.post('/api/horse/feed/', data).then(
          res => {
            let result = res.data;
            if (result.IsSuccess) {
      ...
      sure:function () {
        this.bornDialog = false;
			...
			ex:function(val){
				if(val.GradeLevel==''||val.GradeLevel==null){
					return '0/15';
				}else{
					if(val.Experience==''||val.Experience==null){
							return '0/'+(14+parseInt(val.GradeLevel)^2);
					}else{
						this.exLength=(val.Experience/(14+val.GradeLevel*val.GradeLevel))*100+"%";
						return val.Experience+'/'+(14+val.GradeLevel*val.GradeLevel);
					}
				}
			},
    },
