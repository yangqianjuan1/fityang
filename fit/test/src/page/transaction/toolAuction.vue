<template>
  <div class="toolAuction">
    <div class="title usual" flex="main:center box:mean"  :class="'cur_'+isCur"  >
      <div  @click="choose(0)" :class="isCur===0?'miaobian-hong white':'miaobian-bai'">拍卖行</div>
      <div  @click="choose(1)" :class="isCur===1?'miaobian-hong white':'miaobian-bai'">竞拍</div>
      <div  @click="choose(2)" :class="isCur===2?'miaobian-hong white':'miaobian-bai'">拍卖</div>
    </div>
    <div  v-loading="load">
      <div flex="box:justify cross:center dir:right"  style="background: #F07765;padding:0.2rem 0.5rem">

        <!--<div class="ui-select" flex="main:justify cross:center">-->
        <!--<input  style="height:0.7rem;line-height:0.7rem;text-indent:0.325rem;width:70%;color:#9F9F9F;border-right: 2px solid #7A7A7A;font-size: 14px;border-radius: unset" placeholder="搜索物品" v-model="searchValue"/><i class="search ct" @click="search()"></i>-->
        <!--</div>-->
        <div flex="main:center cross:center">
          <span class="slot miaobian-hong"  @click="priceSlot(!pricePositiveSequence)">价格</span>
          <span :class="pricePositiveSequence?'arrow-up':'arrow-down'" class="arrow" style="margin-left: 5px" ></span>
        </div>
        <div flex="main:center cross:center">
          <span class="slot miaobian-hong"  @click="timeSlot(!secondsPositiveSequence)">剩余时间</span>
          <span :class="secondsPositiveSequence?'arrow-up':'arrow-down' " class="arrow" style="margin-left: 5px"></span>
        </div>
        <selection :values="selectValue" :selectFunc="selectFunc" ref="selectTrans" style="width: 35%" v-if="isCur===0"></selection>
      </div>
      <scroll top="2.5"
              :scrollY="scrollY"
        ...
        <div >
          ...
          <ul  v-if="!empty" >
            <li v-for="(info,index) in horses"  class="horse"  >
              ...
              <button class="Bidding myUnCur  white miaobian-green" @click="Bidding(index,info)" v-if="isCur!==2">竞价</button>
            </li>
          </ul>
          <paging :totalPage="totalPage" :pageFunc="pullUpLoadFuc" ref="page"></paging>
          <div style="position: relative;height: 100%;text-align: center"> <img style="width: 2.5rem;padding: 2rem"  src="../../assets/img/dialog/empty.png" v-if="empty"/></div>
          <!--</contain>-->
        </div>
      </scroll>
    </div>
    ...
    <comfire-dialog v-model="bidDlg" :sure="sureToBidding" :param="biddingInfo">
      <div flex="dir:top main:center cross:center" style="height: 100%" >   <p class="miaobian-hong" style="font-size: 22px;color: #FFE619;text-align: center;margin-bottom: 20px">万利碎片X{{biddingInfo.Count}}</p>

        <div   style="width: 90%;margin: 0 auto" flex="main:left cross:center">
          <!--<input  placeholder="请输入出售金额" v-model.number="sell.price"/>-->
          <!--<span style="color: #fff" class="miaobian-hong">竞拍价格</span>-->
          <div flex="main:center cross:center" style="width: 100%;padding: 0.3rem 0" >
            <!--<button class="minus active" flex="main:center cross:center" @click="toMinus()"></button>-->
            <input type="number"  class="bid miaobian-bai usual"  placeholder="请输入竞拍价格" v-model.number="biddingPrice"/>
            <!--<img src="../../assets/img/transactin/plus.png" >-->
            <!--<button class="plus active" flex="main:center cross:center" @click="toPlus()"></button>-->
          </div>
        </div>
        <p  style="text-align: left;font-size: 14px;color: #ffffff;width: 90%;margin: 0 auto" class="miaobian-hong">每次提价不得低于<span style="color: #FFE619">现价的{{onceBid*100}}%</span></p></div>
    </comfire-dialog>
  </div>
</template>
...
<script>
  ...
  export default{
    ...
    mounted(){
      this.onceBid =this.$store.state.onceBid;
      this.getToolList();
    },
    computed:{
...
//          horseNo:this.searchValue,
        ...
        this.$axios.post('/api/prob/auctionlist/',data).then(
          res=>{
            let result = res.data;
            if(result.IsSuccess){
              ...
              if(result.Result.Data.length>0){
                ...
                for(let horse in horselist ){
                  ...
                  if(!horselist[horse].CurAuctionAccountAmount){
                    horselist[horse].CurAuctionAccountAmount= "未出价";
                  }
                  horselist[horse].IsHighest= null;
                }
...
//          horseNo:this.searchValue,
        ...
        this.$axios.post('/api/prob/myauctionlist/',data).then(
          res=>{
            let result = res.data;
            if(result.IsSuccess){
              if(this.orderByEndTime === 1){
      ...
      getMySellList:function (page) {
        ...
        this.$axios.post('/api/prob/myauctionsalelist/',data).then(
          res=>{
            let result = res.data;
            if(result.IsSuccess){
              if(this.orderByEndTime === 1){
...
//          this.$refs.page.reSetPage();
      ...
      getLevel:function (val) {
        switch (val){
          case '全部':this.level = ''; break;
          case '创世马':this.level = 1; break;
          case '神马':this.level = 1; break;
          case '骏马':this.level = 0; break;
        }
      },//分类
...
//        this.getToolList();
      ...
      sureToBidding:function (info) {
        ...
        this.bidDlg = false;
        let data={tradeKey:info.AuctionKey,amount:this.biddingPrice};
        this.$axios.post('/api/prob/auctionincrease/',data).then(
          res=>{
            let result = res.data;
            if(result.IsSuccess){
              this.bidDlg = false;
...
<style scoped>
  ...
  .active:active{/*点击效果*/   /* For Firefox3.6+ */
