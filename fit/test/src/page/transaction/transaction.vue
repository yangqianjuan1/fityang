    ...
    <div>
      <div flex="main:justify cross:center"
        ...
        <selection :values="selectValue"
                   ...
                   ref="selectTrans"></selection>
        <div class="ui-select"
             flex="main:justify cross:center">
          <input style="height:0.6rem;line-height:0.6rem;text-indent:0.325rem;color:#9F9F9F;width:70%;font-size: 0.37rem;"
                 type="number"
                 class="noStroke"
                 placeholder="搜索ID"
                 v-model.number="searchValue" />
          <img src="../../assets/img/transactin/search.png"
               class="search ct"
               @click="search()">
        </div>
        ...
        <div class="slot"
             @click="priceSlot(!pricePositiveSequence)"
             flex="main:center cross:center">
          <p style="font-size: 0.37rem">价格</p>
          <p :class="pricePositiveSequence?'arrow-up':'arrow-down'"
             class="arrow"
             style="margin-left: 5px"></p>
        </div>
        <img src="../../assets/img/transactin/record.png"
             class="ruleIcon"
             style="margin-right: 0"
             @click="$router.push('/transactionRecord')">
      </div>
      <!--<img src="../../assets/img/transactin/peng.png" style="height: 1rem;width: 100%">-->
      <scroll ref="scrollTrans"
              top="1.5"
              bottom="2">
        <div :style="setScrollMinHeight()">
          ...
          <ul v-if="!empty">
            ...
            <!-- 下面↓↓↓↓↓↓↓↓↓↓↓↓,老代码,是  <li v-for="(info.index) in tigers"  我也不知道index啥用,我也不敢问-->
            <li v-for="(info) in tigers"
                style="width: 33.33%;display: inline-block;padding: 0 0.25rem;"
                @click="transactionDetail(info)"
                :key="info.id">
              <tiger-info :Info="info"
                          class="tigerIcon"></tiger-info>
              <!--<p class="price" style="">{{info.tigerPrice|ETH}}ETH</p>-->
            </li>
          </ul>
          <div style="text-align: center"><img style="width: 2.5rem;padding: 2rem"
                 src="../../assets/img/dialog/empty.png"
                 v-if="empty" /></div>
          <paging :totalPage="totalPage"
                  :pageFunc="getTigerList"
                  ref="page"></paging>
        </div>
      </scroll>
    </div>
...
export default {
  ...
  methods: {
    ...
    getTigerList: function (pageIndex) {
      ...
      this.$axios.post('/api/horse/trade/list/', data).then(
        res => {
          let result = res.data;
          let tigers = [];
          if (result.IsSuccess) {
    ...
    priceSlot: function (type) {
      ...
      this.getTigerList()   // 报错,vuex的问题,暂不知什么问题
