<template>
	<div id="app" :style="{'max-width':$store.state.maxWidth}" ref="quicklink">
		<div>
			<my-magquee :text="notice" v-if="this.$route.meta.notice"></my-magquee>
		...
		<auto-button text="任务" @onFloatBtnClicked="autoButtonFun">
		</auto-button>
		<task-panel v-if="$store.getters.getTaskPanel"></task-panel>
		<web-socket-info ></web-socket-info>
	</div>
</template>
...
<script>
	...
	import TaskPanel from "./page/task/TaskDialog";
	import WebSocketInfo from './components/webSocket.vue'
	export default {
		components: {
			ComfireDialog,
			MyMagquee,
		...
		methods: {
			getNotice: function() {
				this.$axios.get('/api/notice/gethomepagenotice/').then(
					res => {
						let result = res.data;
						if (result.IsSuccess) {
			...
			setMaxWidth: function() {
				...
				if (UA.mobile && !UA.iPad) { //手机端
					...
					this.$store.commit('set_device_type','pc');
				}
