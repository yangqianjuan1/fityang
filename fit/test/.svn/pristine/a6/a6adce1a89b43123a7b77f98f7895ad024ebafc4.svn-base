...
const normalizeTargetCss = mergedOptions => {
    ...
    return css.map((css, i) => {
        ...
        if (Array.isArray(css)) {
            var _css$slice = css.slice(0),
                _css$slice2 = _slicedToArray(_css$slice, 2);

            const cssFileName = _css$slice2[0];
            var _css$slice2$ = _css$slice2[1];
            const options = _css$slice2$ === undefined ? {} : _css$slice2$;

            const format = options.format || extractFormatFromCSSFilename(cssFileName);
            return [cssFileName, _extends({}, options, { format })];
        }
